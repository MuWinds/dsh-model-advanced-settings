window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      ".dsw-mas-level-name{font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);}" +
    ...
    var COPY = {
      en: {
        ...
        levelsHint: "Wire values per model; empty levels are not offered.",
        ...
        emptyLevels: "Enable reasoning and fill at least one level.",
        inherit: "Inherit parent"
      },
      zh: {
        nav: "模型高级设置",
        levelsTitle: "推理等级",
        levelsHint: "每个模型的线上值；留空的等级不会提供。",
        retryTitle: "重试策略",
    ...
    function effortsToLevels(value) {
      if (typeof value === "object" && value !== null && !Array.isArray(value)) {
        ...
        for (var i = 0; i < LEVELS.length; i++) {
          var level = LEVELS[i];
          if (typeof value[level] === "string") out[level] = value[level];
        }
        return out;
      }
      return {};
    ...
    function ModelAdvancedSettingsPage() {
      ...
      function patchLevel(modelId, level, value) {
        setDrafts(function (s) {
          var next = Object.assign({}, s);
          next[route] = (next[route] || []).map(function (d) {
      ...
      function save() {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          for (var j = 0; j < LEVELS.length; j++) {
            var level = LEVELS[j];
            var wire = (row.levels[level] || "").trim();
            if (wire.length > 0) efforts[level] = wire;
          }
            ...
            return react.createElement("div", { key: row.id, className: "dsw-mas-card" },
              ...
              row.enabled ? react.createElement("div", { className: "dsw-mas-levels" },
                LEVELS.map(function (level) {
                  return react.createElement("label", { key: level, className: "dsw-mas-level" },
                    react.createElement("span", { className: "dsw-mas-level-name" }, LEVEL_LABELS[level]),
                    react.createElement("input", {
                      type: "text",
                      className: "dsw-mas-input",
                      value: row.levels[level] || "",
                      placeholder: level,
                      onChange: function (e) { patchLevel(row.id, level, e.target.value); }
                    })
                  );
                })
              ) : null
