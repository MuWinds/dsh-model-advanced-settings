  ...
  factory: (require) => {
    ...
    var COPY = {
      en: {
        ...
        compatTitle: "Provider compatibility",
        compatHint: "pi-ai sends the system prompt under the developer role instead of system. The default detects the answer from the endpoint URL; the switch is stored on the provider and shared by every model it serves.",
        developerRole: "DeveloperRole",
        developerDefault: "Default (auto-detect)",
        developerYes: "Supported",
        developerNo: "Unsupported",
      ...
      zh: {
        ...
        compatTitle: "提供方兼容",
        compatHint: "pi-ai 会把系统提示作为 developer 角色发送（而不是 system）。默认＝按接口地址自动检测；该开关存在提供方上，其下所有模型共用。",
        developerRole: "DeveloperRole",
        developerDefault: "默认（自动检测）",
        developerYes: "支持",
        developerNo: "不支持",
    ...
    /**
     * One provider's stored `compat.supportsDeveloperRole` into a select value.
     *
     * Three states, not two: an absent key is not `false`. pi-ai detects the
     * answer from the endpoint URL, and only the empty choice sends the field
     * back to that detection, so collapsing "unset" onto `false` would turn a
     * default into a declaration the user never made.
     * @param value - the stored switch, or anything else.
     * @returns `"true"`, `"false"`, or `""` for "detect it".
     */
    function developerRoleDraftOf(value) {
    ...
    function ModelAdvancedSettingsPage() {
      ...
      var compat = _useState12[0];
      var setCompat = _useState12[1];
      ...
      react.useEffect(function () {
        ...
        api("load", {}).then(function (result) {
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            c[rr.route] = developerRoleDraftOf(rr.supportsDeveloperRole);
          }
      ...
      /** Rewrite one provider's DeveloperRole choice. */
      function setDeveloperRole(rt, value) {
        setCompat(function (s) {
          var next = Object.assign({}, s);
          next[rt] = value;
          return next;
        });
      ...
      function save() {
        ...
        setMessage(null);
        setOk(null);
        // The empty choice is sent as `null` — the request's "state no answer"
        // — and the host turns it into an `unset`, so a provider that never
        // declared the switch keeps declaring nothing.
        var roleDraft = compat[route] === undefined ? "" : compat[route];
        api("save", {
          route: route,
          models: models,
          retryPolicy: retryPolicy,
          supportsDeveloperRole: roleDraft === "true" ? true : roleDraft === "false" ? false : null
        }).then(function () {
          setSaving(false);
          setOk(true);
          setMessage(t.saved);
      ...
      var developerRole = compat[route] === undefined ? "" : compat[route];
      ...
      return react.createElement("div", { className: "dsw-mas-root" },
        ...
        react.createElement("div", { className: "dsw-mas-block" },
          ...
          react.createElement("div", { className: "dsw-mas-block" },
            react.createElement("h3", { className: "dsw-mas-block-title" }, t.compatTitle),
            react.createElement("p", { className: "dsw-mas-block-hint" }, t.compatHint),
            react.createElement("div", { className: "dsw-mas-row" },
              react.createElement("label", { className: "dsw-mas-label" },
                react.createElement("span", null, t.developerRole),
                react.createElement("select", {
                  className: "dsw-mas-input dsw-mas-select dsw-mas-compat",
                  value: developerRole,
                  onChange: function (e) { setDeveloperRole(route, e.target.value); setMessage(null); setOk(null); }
                },
                  react.createElement("option", { value: "" }, t.developerDefault),
                  react.createElement("option", { value: "true" }, t.developerYes),
                  react.createElement("option", { value: "false" }, t.developerNo)
                )
              )
            )
          ),
